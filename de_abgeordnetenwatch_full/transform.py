...
def make_fraction_membership(
    context: Context, person: CE, data: dict[str, Any]
) -> CEGenerator:
...
def handle(context: Context, record: dict[str, Any], ix: int) -> CEGenerator:
    parliament_period_detail = record.pop("period")
    parliament_detail = record.pop("parliament")
    politician_detail = record.pop("politician")
    mandate = record.pop("mandate")
    ...
    person.id = context.make_slug("person", politician_detail.pop("id"))

    person.add("wikidataId", politician_wikidata_id)
    person.add("birthDate", politician_detail.pop("year_of_birth"))
    person.add("name", politician_detail.pop("label"))
    person.add("firstName", politician_detail.pop("first_name"))
    person.add("lastName", politician_detail.pop("last_name"))
    person.add("motherName", politician_detail.pop("birth_name"))
    person.add("education", politician_detail.pop("education"))
    person.add("description", politician_detail.pop("occupation"))
    person.add("sourceUrl", politician_detail.pop("abgeordnetenwatch_url"))

    record_start_date = mandate.pop("start_date") or parliament_period_detail.pop(
        "start_date_period"
    )
    record_end_date = mandate.pop("end_date") or parliament_period_detail.pop(
        "end_date_period"
    )
    ...
    party_name = ensure_dict(politician_detail.pop("party")).get("label")
    if party_name is not None:
        yield from make_party_membership(context, person, party_name)

    for membership in ensure_list(mandate.pop("fraction_membership", None)):
        yield from make_fraction_membership(context, person, membership)
